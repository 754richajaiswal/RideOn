import React from 'react'
import { Link } from 'react-router-dom'
import '../Style/captainlogin.css'
import log from '../assets/2.jpg'

const CaptainLogin = () => {
  return (
    <div className='user_login'>
          <div className="userlogin">
            <h2>Login</h2>
            <div className="buttons">
              <Link to='/userlogin'><button>Rider</button></Link>
              <Link to='/captainlogin'><button>Driver</button></Link>
            </div>
          </div>
          <div className="userinfo">
          <div className="image">
            <img src={log} alt="" />
          </div>
    
          {/* <h2 id='rider'>Register as a Rider</h2> */}
          <div className="info">
              <input type="text" placeholder='Email Or Mobile Number' required />
              <input type="text" placeholder='Password' required />
            <div className="link">
              <Link to='/usersignup'className='underline' id='line'><h5>Don't have an account? Register</h5></Link>
              <Link to='/forgotpassword' className='underline'><h5>Forgot Password?</h5></Link>
            </div>
              <button className='captain_login'>Login</button>
          </div>
          <h5 id='or'>OR</h5>
          <button id='google'><img src="data:image/png;base64," alt="" /> Continue with Google</button>
          </div>
        </div>
  )
}

export default CaptainLogin